...
export default function Header({ currentLang, onLangChange }: HeaderProps) {
  ...
  const subscribeText = currentLang === 'zh' ? "訂閱子瓏" : currentLang === 'ja' ? "子瓏（ズーロン）を購読する" : "Subscribe to Zi Long";
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-50 border-b border-accent/10 bg-bg/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="hidden md:block mono-label">
              {currentLang === 'zh' ? "理性的邊界建構者" : currentLang === 'ja' ? "理性的な境界の構築者" : "Rational Boundary Builder"}
            </span>
          </div>
              ...
              <div className="w-full max-w-xs space-y-4" onClick={e => e.stopPropagation()}>
                <div className="text-center mb-8">
                  ...
                  <h3 className="text-xl font-serif">
                    {currentLang === 'zh' ? "選擇語言" : currentLang === 'ja' ? "言語を選択" : "Select Language"}
                  </h3>
                </div>
                {languages.map((lang) => (
                  <button
                ...
                >
                  {currentLang === 'zh' ? "關閉" : currentLang === 'ja' ? "閉じる" : "Close"}
                </button>
              </div>
            </motion.div>
          )}
      ...
      <SubscriptionModal isOpen={isSubModalOpen} onClose={() => setIsSubModalOpen(false)} lang={currentLang} />
    </>
  );
}
